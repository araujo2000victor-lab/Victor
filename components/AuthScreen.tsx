...
const AuthScreen: React.FC<AuthScreenProps> = ({ onLoginSuccess }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      if (isRegistering) {
        ...
        if (username.length < 3) throw new Error("Nome de guerra muito curto.");
  ...
  const selectExistingUser = (user: User) => {
       ...
       <div className="w-full max-w-md z-10 animate-in fade-in zoom-in duration-500">
         ...
         <div className="bg-white dark:bg-bope-gray border border-gray-200 dark:border-gray-800 shadow-xl rounded-sm p-8">
            <h2 className="text-lg font-bold uppercase mb-6 flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 pb-2">
              ...
              {isRegistering ? 'Alistamento Militar' : 'Acesso ao Quartel'}
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
               ...
               <div>
                 ...
                 <div className="relative">
                   ...
                   <input 
                     ...
                     required
                     autoFocus={!isRegistering && existingUsers.length === 0}
                   />
                 </div>
               </div>
               ...
               >
                 {isRegistering ? 'Confirmar Alistamento' : 'Autenticar'} <ChevronRight className="w-4 h-4" />
               </button>
            </form>
            ...
            <div className="mt-6 text-center border-t border-gray-100 dark:border-gray-700 pt-4">
              ...
              >
                {isRegistering ? 'Já tem cadastro? Entrar no Quartel' : 'Novo Recruta? Alistar-se'}
              </button>
            </div>
         </div>
         ...
         {!isRegistering && existingUsers.length > 0 && (
             <div className="mt-8 bg-white dark:bg-bope-gray border border-gray-200 dark:border-gray-800 shadow-md rounded-sm p-4 animate-in slide-in-from-bottom-2">
                 <h3 className="text-[10px] font-bold uppercase text-gray-400 mb-3 text-center">Identidades Salvas</h3>
                 <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                     {existingUsers.map(u => (
                         <div key={u.id} className="flex justify-between items-center p-2 bg-gray-50 dark:bg-black/20 rounded-sm border border-gray-100 dark:border-gray-700 hover:border-emerald-500 transition-colors cursor-pointer group" onClick={() => selectExistingUser(u)}>
                             <div className="flex items-center gap-3">
                                 <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden border border-gray-300 dark:border-gray-600">
                                     {u.avatar ? <img src={u.avatar} className="w-full h-full object-cover" alt="avatar"/> : <UserIcon className="w-4 h-4 m-2 text-gray-500"/>}
                                 </div>
                                 <div className="flex flex-col">
                                     <span className="text-xs font-mono font-bold uppercase text-gray-700 dark:text-gray-200">{u.username}</span>
                                     <span className="text-[9px] text-gray-400">{u.rank || 'Recruta'}</span>
                                 </div>
                             </div>
